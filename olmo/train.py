...
class Trainer:
    cfg: TrainConfig
    model: Union[OLMo, OlmoeForCausalLM]
    dist_model: Union[DDP, FSDP]
    teacher_model: Optional[OLMo]  
    optim: Optimizer
    scheduler: Scheduler
    ...
    def model_forward(
        ...
        # Distillation loss (distill from OLMo to scratch descendant model)
        self.kd_ratio = 0.5
        if self.teacher_model is not None: 
            with torch.no_grad():
                self.teacher_model.eval()
                teacher_outputs = self.teacher_model(input_ids=batch["input_ids"], attention_mask=batch.get("attention_mask"), use_cache=False)
            ...
            mask = (labels_for_distill != -100).int()
            distil_loss = -torch.sum(x * mask.view(-1), dim=0) / torch.sum(mask.view(-1), dim=0)       
            ce_loss = (1 - self.kd_ratio) * ce_loss + self.kd_ratio * distil_loss
