...
sys.path.append("/home/liuchang/code/OLMo-main/")
...
from olmo.olmoe_utils import get_olmoe_model
...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.model.moe:
        olmo_model = get_olmoe_model(cfg)
    else:
    ...
    if (param_init_fn is not None or cfg.distributed_strategy == DistributedStrategy.ddp) and not cfg.model.moe:
    ...
    ### build teacher model
    if cfg.distill:
        teacher_model = AutoModelForCausalLM.from_pretrained(cfg.ancestor_path, torch_dtype=torch.float16)
